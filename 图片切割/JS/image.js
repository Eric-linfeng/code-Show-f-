// 需求分析 1：点击开始按钮，开始游戏，同时开始开始按钮变成复原，点击复原，游戏复原！
// 2：点击简单按钮，弹出不同的困难选项，再次点击，列表消失
// 3.选择不同的困难，改变分割的多少，同时列表消失！
...
function gameState() {
    ...
    $('.btn').click(function () {
        // 点击开始按钮，开始游戏，同时开始开始按钮变成复原，点击复原，游戏复原！
        if (key) {
            key = false;
            $(this).text('复原')
            render(num)
...
function select() {
    // 2：点击简单按钮，弹出不同的困难选项，再次点击，列表消失
    // 2.1给自己添加active，其他不选择
    // 2.2给li添加点击事件，同时谁点击谁移除，选择不同的难度，返回不同的值
